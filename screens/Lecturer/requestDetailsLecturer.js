      ...
      try {
        ...
        if (token) {
          const { data } = await axios.get(
            `${process.env.EXPO_PUBLIC_API_URL}/api/request/single/lecturer/${requestId}`,
            {
              headers: {
          ...
          setResponse(data);
          if (response.receiverReply) {
            setReplied(true);
            setTextAreaValue(response.receiverReply);
          }
        }
      } catch (error) {
  ...
  const handleSendData = async () => {
    try {
      const response = await axios.put(
        `${process.env.EXPO_PUBLIC_API_URL}/api/request/reply/${requestId}`,
        { reply: textAreaValue },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status >= 200 && response.status < 300) {
        setSuccessMessage("Reply was sent successfully"); // Set success message
      } else {
        setSuccessMessage("An error occurred."); // Set error message
      }
    } catch (error) {
          ...
          <ScrollView contentContainerStyle={styles.scrollViewContainer}>
            ...
            <View style={styles.replyContainer}>
              {!replied ? (
                ...
                >
                  {response?.receiverReply ? response?.receiverReply : ""}
                </TextInput>
              ) : (
                <TextInput
                  style={styles.replyInput}
